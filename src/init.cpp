...
void Shutdown()
{
    ...
    RenameThread("mybasecoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Mybasecoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Mybasecoin.pid)") + "\n";
    ...
    strUsage += "\n" + _("SSL options: (see the Mybasecoin Wiki for SSL setup instructions)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeionamount=<n> " + _("Keep N Mybasecoin anonymized (default: 0)") + "\n";
...
std::string LicenseInfo()
{
    ...
    const std::string URL_WEBSITE = "<https://www.mybasecoin.org>";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Mybasecoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Mybasecoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Mybasecoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon) fprintf(stdout, "Mybasecoin server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Mybasecoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Mybasecoin to complete") << "\n";
    ...
    LogPrintf("Anonymize Mybasecoin Amount %d\n", nAnonymizeSyndicateAmount);
